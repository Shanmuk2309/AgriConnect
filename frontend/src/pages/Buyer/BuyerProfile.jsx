...
const BuyerProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (!data.address) {
          data.address = { shop_no: '', street: '', city: '', district: '', state: '', pincode: '' };
        }
        setProfileData(data);
      } catch (error) {
  ...
  const handleSave = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`/api/buyers/${userId}`, profileData);
      setIsEditing(false);
  ...
  const businessName = profileData.business_name || profileData.name || 'Buyer';
          ...
          <div className="profile-card">
            {isEditing ? (
              ...
              <div className="profile-details">
                <div className="detail-group"><span className="label">Full Name:</span> <span className="value">{profileData.name}</span></div>
